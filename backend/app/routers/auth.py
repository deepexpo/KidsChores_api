...
from app.services.linking import redeem_link_code
...
_LINK_IP_LIMIT = (10, 15 * 60)  # 10 attempts / 15 min / IP
_LINK_CODE_LIMIT = (5, 15 * 60)  # 5 attempts / 15 min / code — a 6-digit code is brute-forceable
...
async def link_account(
    ...
    """
    Redeems a link code (generated by a parent via
    POST /v1/household/members/{id}/link-code) to attach the caller's own
    email + password to an existing teen profile — same member id, same
    household, same role, just replacing the placeholder auth_subject with a
    real one. This is how a teen ends up able to log in on their own device
    instead of only via shared-device PIN mode; registering via /auth/register
    would instead create a brand-new household, which is wrong for a teen who
    already has a profile.
    """
    ip = _client_ip(request)
    try:
        await check_rate_limit(f"link:ip:{ip}", *_LINK_IP_LIMIT)
        await check_rate_limit(f"link:code:{body.code}", *_LINK_CODE_LIMIT)
    except RateLimitExceededError as exc:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Too many attempts. Try again later.",
    ...
    member_id = await redeem_link_code(body.code)
    ...
    member = result.scalar_one_or_none()
    if member is None or member.role != MemberRole.teen:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Invalid or expired code.",
        )
    if not member.auth_subject.startswith(f"pending-{member.household_id}-"):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="This profile already has its own login.",
        )
